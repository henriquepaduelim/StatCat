...
def check_athlete(athlete_id=30):
    ...
    cursor.execute("SELECT id, first_name, last_name, email FROM athlete WHERE id = ?", (athlete_id,))
    ...
    if not user:
        ...
        if status == "APPROVED":
            ...
            print(f"   UPDATE user SET athlete_status = 'INCOMPLETE' WHERE athlete_id = {athlete_id};")
