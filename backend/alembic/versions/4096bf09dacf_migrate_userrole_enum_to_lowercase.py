...
def upgrade() -> None:
    ...
    op.execute("CREATE TYPE userrole_new AS ENUM ('athlete', 'coach', 'staff', 'admin')")
    ...
    op.execute('ALTER TABLE "user" ALTER COLUMN role TYPE userrole_new USING lower(role::text)::userrole_new')
...
def downgrade() -> None:
    ...
    op.execute("CREATE TYPE userrole_old AS ENUM ('ATHLETE', 'COACH', 'STAFF', 'ADMIN')")
    op.execute('ALTER TABLE "user" ALTER COLUMN role DROP DEFAULT')
    op.execute('ALTER TABLE "user" ALTER COLUMN role TYPE userrole_old USING upper(role::text)::userrole_old')
    op.execute("DROP TYPE userrole")
