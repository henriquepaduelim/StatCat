...
def upgrade() -> None:
    ...
    op.execute(
        "CREATE TYPE userathleteapprovalstatus_new AS ENUM ('pending', 'approved', 'rejected', 'incomplete')"
    )
    op.execute(
        'ALTER TABLE "user" ALTER COLUMN athlete_status TYPE userathleteapprovalstatus_new USING lower(athlete_status::text)::userathleteapprovalstatus_new'
    )
    op.execute("DROP TYPE userathleteapprovalstatus")
    op.execute(
        "ALTER TYPE userathleteapprovalstatus_new RENAME TO userathleteapprovalstatus"
    )

    # participantstatus -> lowercase
    op.execute(
        "CREATE TYPE participantstatus_new AS ENUM ('invited', 'confirmed', 'declined', 'maybe')"
    )
    op.execute("ALTER TABLE event_participant ALTER COLUMN status DROP DEFAULT")
    op.execute(
        "ALTER TABLE event_participant ALTER COLUMN status TYPE participantstatus_new USING lower(status::text)::participantstatus_new"
    )
    ...
    op.execute(
        "CREATE TYPE reportsubmissionstatus_new AS ENUM ('pending', 'approved', 'rejected')"
    )
    op.execute("ALTER TABLE report_submission ALTER COLUMN status DROP DEFAULT")
    op.execute(
        "ALTER TABLE report_submission ALTER COLUMN status TYPE reportsubmissionstatus_new USING lower(status::text)::reportsubmissionstatus_new"
    )
    ...
    op.execute(
        "CREATE TYPE combinemetricstatus_new AS ENUM ('pending', 'approved', 'rejected')"
    )
    op.execute("ALTER TABLE team_combine_metric ALTER COLUMN status DROP DEFAULT")
    op.execute(
        "ALTER TABLE team_combine_metric ALTER COLUMN status TYPE combinemetricstatus_new USING lower(status::text)::combinemetricstatus_new"
    )
...
def downgrade() -> None:
    ...
    op.execute(
        "CREATE TYPE combinemetricstatus_old AS ENUM ('PENDING', 'APPROVED', 'REJECTED')"
    )
    op.execute("ALTER TABLE team_combine_metric ALTER COLUMN status DROP DEFAULT")
    op.execute(
        "ALTER TABLE team_combine_metric ALTER COLUMN status TYPE combinemetricstatus_old USING upper(status::text)::combinemetricstatus_old"
    )
    op.execute("DROP TYPE combinemetricstatus")
    ...
    op.execute(
        "CREATE TYPE reportsubmissionstatus_old AS ENUM ('PENDING', 'APPROVED', 'REJECTED')"
    )
    op.execute("ALTER TABLE report_submission ALTER COLUMN status DROP DEFAULT")
    op.execute(
        "ALTER TABLE report_submission ALTER COLUMN status TYPE reportsubmissionstatus_old USING upper(status::text)::reportsubmissionstatus_old"
    )
    op.execute("DROP TYPE reportsubmissionstatus")
    ...
    op.execute(
        "CREATE TYPE participantstatus_old AS ENUM ('INVITED', 'CONFIRMED', 'DECLINED', 'MAYBE')"
    )
    op.execute("ALTER TABLE event_participant ALTER COLUMN status DROP DEFAULT")
    op.execute(
        "ALTER TABLE event_participant ALTER COLUMN status TYPE participantstatus_old USING upper(status::text)::participantstatus_old"
    )
    op.execute("DROP TYPE participantstatus")
    ...
    op.execute(
        "CREATE TYPE userathleteapprovalstatus_old AS ENUM ('INCOMPLETE', 'PENDING', 'APPROVED', 'REJECTED')"
    )
    op.execute(
        'ALTER TABLE "user" ALTER COLUMN athlete_status TYPE userathleteapprovalstatus_old USING upper(athlete_status::text)::userathleteapprovalstatus_old'
    )
    op.execute("DROP TYPE userathleteapprovalstatus")
    op.execute(
        "ALTER TYPE userathleteapprovalstatus_old RENAME TO userathleteapprovalstatus"
    )